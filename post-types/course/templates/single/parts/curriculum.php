    ...
    <div class="eltdf-course-curriculum">
        ...
        <div class="eltdf-course-curriculum-list">
			<?php foreach ( $course_sections as $course_section ) {
				...
				?>
                <div class="eltdf-curriculum-section" style="position: relative;z-index: 1">
                    <!--todo head of section -->
					<?php if ( $section_lock['is_readable'] && $section_lock['is_locked'] ) { ?>
                        <div class="ecomhub-fi-locked-section"
                             style="z-index: 999998;position: absolute;width: 100%;height: 100%; top:0;left: 0; background-color: transparent; opacity: 0.25"
                             data-section="<?= $section_counter ?>" data-oo="curriculum"></div>
                        <div class="ecomhub-fi-locked-description"
                             style="z-index: 999999;position: absolute;width: 18em;height: 6em; top:40%;left: 30% ">
                            <div style="text-align: center;padding: 0.5em;background-color: #fbfbfb">
                                <span class="ecomhub-fi-lock-image fa fa-lock"
                                      style="color:#3A4242;font-size: 3em"></span>
                                <br>
                                <span class="ecomhub-fi-lock-human-span"
                                      style="color: black"><?= $section_lock['human'] ?></span>
                            </div>
                        </div>
						<?php
					}
                                ...
                                <div class="eltdf-section-elements">
									...
									foreach ( $elements as $key => $element ) {
										...
										if ( $b_completed ) {
											...
											$completed_html = $incompleted_html_template;
										}
										?>
                                        <div class="eltdf-section-element <?php echo esc_attr( $element['class'] ); ?> clearfix <?php echo eltdf_lms_get_course_item_completed_class( $element['id'] ); ?>"
                                             data-section-element-id="<?php echo esc_attr( $element['id'] ); ?>">
                                            <div class="eltdf-element-title">
												...
												<?php if ( eltdf_lms_course_is_preview_available( $element['id'] ) && ( $section_lock['is_readable'] ) && ( ! $section_lock['is_locked'] ) ) { ?>
                                                    <a class="eltdf-element-name eltdf-element-link-open" itemprop="url"
                                                       href="<?php echo esc_attr( $element['url'] ); ?>"
                                                       title="<?php echo esc_attr( $element['title'] ); ?>"
                                                       data-item-id="<?php echo esc_attr( $element['id'] ); ?>"
