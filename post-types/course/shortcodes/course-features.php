...
class CourseFeatures implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map( array(
					'name'                      => esc_html__( 'Elated Course Features', 'eltdf-lms' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						array(
							...
							'heading'     => esc_html__( 'Selected Course', 'eltdf-lms' ),
							'settings'    => array(
								'sortable'      => true,
								'unique_values' => true
							),
							'description' => esc_html__( 'If you left this field empty then course ID will be of the current page', 'eltdf-lms' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'course_duration',
							'heading'     => esc_html__( 'Show Course Duration', 'eltdf-lms' ),
						...
						array(
							...
							'heading'     => esc_html__( 'Show Course Units', 'eltdf-lms' ),
							'value'       => array_flip( esmarts_elated_get_yes_no_select_array( false, true ) ),
							'admin_label' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Show Course Students', 'eltdf-lms' ),
							'value'       => array_flip( esmarts_elated_get_yes_no_select_array( false, true ) ),
							'admin_label' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Show Course Passing Percentage', 'eltdf-lms' ),
							'value'       => array_flip( esmarts_elated_get_yes_no_select_array( false, true ) ),
							'admin_label' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Show Course Maximum Retakes', 'eltdf-lms' ),
							'value'       => array_flip( esmarts_elated_get_yes_no_select_array( false, true ) ),
							'admin_label' => true
						)
					)
	...
	public function getCourseDurationHtml( $params ) {
		...
		if ( $params['course_duration'] == 'yes' && $duration_value != '' ) {
			...
			$html .= '<span class="eltdf-item-icon"><i class="lnr lnr-clock" aria-hidden="true"></i></span>';
			$html .= '<span class="eltdf-item-label">' . esc_html__( 'Duration', 'eltdf-lms' ) . '</span>';
	...
	public function getCourseUnitsHtml( $params ) {
		...
		$course_lectures = eltdf_lms_get_items_in_course( $course_id );
		...
		if ( $params['course_units'] == 'yes' ) {
			$html = '<li class="eltdf-feature-item">';
			$html .= '<span class="eltdf-item-icon"><i class="lnr lnr-alarm" aria-hidden="true"></i></span>';
			$html .= '<span class="eltdf-item-label">' . esc_html__( 'Lectures', 'eltdf-lms' ) . '</span>';
			$html .= '<span class="eltdf-item-value">' . $lesson_count . '</span>';
			$html .= '</li>';
			
			$html .= '<li class="eltdf-feature-item">';
			$html .= '<span class="eltdf-item-icon"><i class="lnr lnr-dice" aria-hidden="true"></i></span>';
			$html .= '<span class="eltdf-item-label">' . esc_html__( 'Quizzes', 'eltdf-lms' ) . '</span>';
	...
	public function getCoursePassPercentageHtml( $params ) {
		...
		if ( $params['course_pass_percent'] == 'yes' && $pass_percentage != '' ) {
			$html = '<li class="eltdf-feature-item">';
			$html .= '<span class="eltdf-item-icon"><i class="fa fa-thumbs-o-up" aria-hidden="true"></i></span>';
			$html .= '<span class="eltdf-item-label">' . esc_html__( 'Pass Percentage', 'eltdf-lms' ) . '</span>';
	...
	public function getCourseMaxRetakesHtml( $params ) {
		...
		if ( $params['course_retakes'] == 'yes' && $max_retakes != '' ) {
			$html = '<li class="eltdf-feature-item">';
			$html .= '<span class="eltdf-item-icon"><i class="fa fa-thumbs-o-up" aria-hidden="true"></i></span>';
			$html .= '<span class="eltdf-item-label">' . esc_html__( 'Max Retakes', 'eltdf-lms' ) . '</span>';
