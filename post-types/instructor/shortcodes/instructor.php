...
class Instructor implements Lib\ShortcodeInterface {
			...
			vc_map( array(
					...
					'params'                    => array(
						...
						array(
							...
							'heading'     => esc_html__( 'Select Instructor', 'eltdf-lms' ),
							'settings'    => array(
								'sortable'      => true,
								'unique_values' => true
							),
							'description' => esc_html__( 'If you left this field empty then project ID will be of the current page', 'eltdf-lms' )
						)
					)
				)
			);
	...
	public function render( $atts, $content = null ) {
		...
		$html = '<div class="eltdf-instructor-item-holder">';
		$html .= eltdf_lms_get_cpt_shortcode_module_template_part( 'instructor', 'instructor-template-' . $params['instructor_layout'], '', $params );
		$html .= '</div>';
