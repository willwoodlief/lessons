...
//Register meta boxes
if ( ! function_exists( 'eltdf_lms_quiz_meta_box_functions' ) ) {
	function eltdf_lms_quiz_meta_box_functions( $post_types ) {
		$post_types[] = 'quiz';
		
		return $post_types;
	}
	
	add_filter( 'esmarts_elated_filter_meta_box_post_types_save', 'eltdf_lms_quiz_meta_box_functions' );
	add_filter( 'esmarts_elated_filter_meta_box_post_types_remove', 'eltdf_lms_quiz_meta_box_functions' );
}

//Register meta boxes scope
if ( ! function_exists( 'eltdf_lms_quiz_scope_meta_box_functions' ) ) {
	function eltdf_lms_quiz_scope_meta_box_functions( $post_types ) {
		$post_types[] = 'quiz';
		
		return $post_types;
	}
	
	add_filter( 'esmarts_elated_filter_set_scope_for_meta_boxes', 'eltdf_lms_quiz_scope_meta_box_functions' );
}

//Register quiz post type
if ( ! function_exists( 'eltdf_lms_register_quiz_cpt' ) ) {
	function eltdf_lms_register_quiz_cpt( $cpt_class_name ) {
		$cpt_class = array(
			'ElatedfLMS\CPT\Quiz\QuizRegister'
		);
	...
	add_filter( 'eltdf_lms_filter_register_custom_post_types', 'eltdf_lms_register_quiz_cpt' );
}

//Quiz single functions
if ( ! function_exists( 'eltdf_lms_get_single_quiz' ) ) {
	function eltdf_lms_get_single_quiz() {
		eltdf_lms_get_cpt_single_module_template_part( 'templates/single/holder', 'quiz', '', array() );
	}
}

//Getter and setter for user quiz values
if ( ! function_exists( 'eltdf_lms_set_user_quiz_values' ) ) {
	function eltdf_lms_set_user_quiz_values( $course_id = 0, $quiz_id = 0, $params = array(), $field = '' ) {
...
if ( ! function_exists( 'eltdf_lms_get_user_quiz_values' ) ) {
	function eltdf_lms_get_user_quiz_values( $course_id = 0, $quiz_id = 0, $field = '' ) {
		$user_quizzes = get_user_meta( get_current_user_id(), $field, true );
		$params       = array();
...
if ( ! function_exists( 'eltdf_lms_get_quiz_status_meta_params' ) ) {
	function eltdf_lms_get_quiz_status_meta_params( $quiz_id = 0, $params = array() ) {
		extract( $params );
...
if ( ! function_exists( 'eltdf_lms_get_quiz_results_meta_params' ) ) {
	function eltdf_lms_get_quiz_results_meta_params( $retake_id = 0, $params = array() ) {
		extract( $params );
...
if ( ! function_exists( 'eltdf_lms_get_quiz_temp_meta_params' ) ) {
	function eltdf_lms_get_quiz_temp_meta_params( $quiz_id = 0, $params = array() ) {
		extract( $params );
...
//Action functions
if ( ! function_exists( 'eltdf_lms_start_quiz' ) ) {
	function eltdf_lms_start_quiz() {
		if ( empty( $_POST ) || ! isset( $_POST ) ) {
			...
			if ( $quiz_temp == '' || empty( $quiz_temp ) ) {
				...
				$params_results = eltdf_lms_get_quiz_results_meta_params( $retake );
				eltdf_lms_set_user_quiz_values( $course_id, $quiz_id, $params_results, 'eltdf_user_quiz_results' );
				
				//Update temporary field
				$params_results = eltdf_lms_get_quiz_temp_meta_params( $quiz_id );
				eltdf_lms_set_user_quiz_values( $course_id, $quiz_id, $params_results, 'eltdf_user_quiz_temp' );
			}
	...
	add_action( 'wp_ajax_eltdf_lms_start_quiz', 'eltdf_lms_start_quiz' );
}

if ( ! function_exists( 'eltdf_lms_load_first_question' ) ) {
	function eltdf_lms_load_first_question() {
		if ( empty( $_POST ) || ! isset( $_POST ) ) {
			eltdf_lms_ajax_status( 'error', esc_html__( 'All fields are empty', 'eltdf-lms' ) );
		} else {
			...
			$html                           = eltdf_lms_cpt_single_module_template_part( 'templates/single/holder', 'quiz', 'active', $params );
			$json_data['html']              = $html;
	...
	add_action( 'wp_ajax_eltdf_lms_load_first_question', 'eltdf_lms_load_first_question' );
}

if ( ! function_exists( 'eltdf_lms_finish_quiz' ) ) {
	function eltdf_lms_finish_quiz() {
		if ( empty( $_POST ) || ! isset( $_POST ) ) {
			eltdf_lms_ajax_status( 'error', esc_html__( 'All fields are empty', 'eltdf-lms' ) );
		} else {
			$data        = $_POST;
			...
			parse_str( $data_string, $data_array );
			$quiz_id        = $data_array['eltdf_lms_quiz_id'];
			$item_id        = $data_array['eltdf_lms_quiz_id'];
			...
			$time_total_formatted  = eltdf_lms_get_quiz_time_formated( $quiz_id, $time_remaining );
			$params_status['time'] = $time_total_formatted;
			
			$result        = eltdf_lms_calculate_result( $course_id, $quiz_id );
			$params_status = array_merge( $params_status, $result );
			
			eltdf_lms_set_user_quiz_values( $course_id, $quiz_id, $params_status, 'eltdf_user_quiz_status' );
			...
			$html                   = eltdf_lms_cpt_single_module_template_part( 'templates/single/holder', 'quiz', '', $params );
			$json_data['html']      = $html;
	...
	add_action( 'wp_ajax_eltdf_lms_finish_quiz', 'eltdf_lms_finish_quiz' );
}

if ( ! function_exists( 'eltdf_lms_get_quiz_time_formated' ) ) {
	function eltdf_lms_get_quiz_time_formated( $quiz_id = 0, $time_remaining = 0 ) {
...
if ( ! function_exists( 'eltdf_lms_calculate_result' ) ) {
	function eltdf_lms_calculate_result( $course_id = 0, $quiz_id = 0 ) {
		$quiz_temp = eltdf_lms_get_user_quiz_values( $course_id, $quiz_id, 'eltdf_user_quiz_temp' );
		...
		foreach ( $questions as $question ) {
			...
			if ( isset( $result_params ) ) {
				...
				if ( ! isset( $answer_value ) || $answer_value == '' ) {
					...
					$result = eltdf_lms_validate_question( $question, $answer_value, true );
					$points += $result;
					if ( $result == 0 ) {
						$q_wrong ++;
					} else {
...
//Template functions
if ( ! function_exists( 'eltdf_lms_template_start_quiz_button' ) ) {
	function eltdf_lms_template_start_quiz_button( $params = array() ) {
		$quiz_id      = $params['item_id'];
		$course_id    = $params['course_id'];
		$quiz_results = eltdf_lms_get_user_quiz_values( $course_id, $quiz_id, 'eltdf_user_quiz_results' );
		$disabled     = '';
		...
		if ( ! empty( $quiz_results ) ) {
			...
			if ( $retakes_allowed != '' && $retakes_taken - 1 <= $retakes_allowed ) {
				...
				$button_text    = esc_html__( 'Start quiz', 'eltdf-lms' ) . ' +' . esc_attr( $retakes_remain );
				$custom_attrs   = array();
			} else {
				$retakes_taken = - 1;
				$button_text   = esc_html__( 'No attempts allowed', 'eltdf-lms' );
				$custom_attrs  = array(
					'disabled' => 'disabled'
				);
				$disabled      = 'disabled';
			...
			$retakes_taken = 0;
			$button_text   = esc_html__( 'Start quiz', 'eltdf-lms' );
			$custom_attrs  = array();
		}
		...
		if ( $quiz_temp != '' && ! empty( $quiz_temp ) ) {
			...
			$button_text   = esc_html__( 'Resume quiz', 'eltdf-lms' );
			$custom_attrs  = array();
		}
		...
		return eltdf_lms_get_cpt_single_module_template_part( 'templates/single/parts/actions-start', 'quiz', '', $start_form_params );
	}
}

if ( ! function_exists( 'eltdf_lms_template_quiz_info_top' ) ) {
	function eltdf_lms_template_quiz_info_top( $params = array() ) {
		$questions               = get_post_meta( get_the_ID(), 'eltdf_quiz_question_meta', true );
		$questions_number        = ! empty( $questions ) ? count( $questions ) : 0;
		$questions_label         = $questions_number == 1 ? esc_html__( 'Question', 'eltdf-lms' ) : esc_html__( 'Questions', 'eltdf-lms' );
		...
		$quiz_temp = eltdf_lms_get_user_quiz_values( $params['course_id'], $params['item_id'], 'eltdf_user_quiz_temp' );
		if ( $quiz_temp != '' && ! empty( $quiz_temp ) ) {
			if ( $quiz_temp['time_remaining'] != 0 ) {
		...
		$info_top_params = array(
			...
			'quiz_duration_parameter' => esc_html__( '(hh:mm:ss)', 'eltdf-lms' )
		);
		
		return eltdf_lms_get_cpt_single_module_template_part( 'templates/single/parts/info-top', 'quiz', '', $info_top_params );
	}
}

if ( ! function_exists( 'eltdf_lms_template_quiz_status' ) ) {
	function eltdf_lms_template_quiz_status( $params = array() ) {
		$quiz_id   = $params['item_id'];
		$course_id = $params['course_id'];
		
		$quiz_status = eltdf_lms_get_user_quiz_values( $course_id, $quiz_id, 'eltdf_user_quiz_status' );
		...
		return eltdf_lms_get_cpt_single_module_template_part( 'templates/single/parts/status', 'quiz', '', $status_params );
	}
}

if ( ! function_exists( 'eltdf_lms_template_quiz_results' ) ) {
	function eltdf_lms_template_quiz_results( $params = array() ) {
		$quiz_id   = $params['item_id'];
		$course_id = $params['course_id'];
		$empty     = true;
		...
		return eltdf_lms_get_cpt_single_module_template_part( 'templates/single/parts/results', 'quiz', '', $results_params );
	}
}